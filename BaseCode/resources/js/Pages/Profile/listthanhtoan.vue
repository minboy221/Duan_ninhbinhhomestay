...
const startPolling = () => {
    ...
    pollTimer = setInterval(async () => {
        ...
        try {
            const res = await axios.get(`/api/invoices/${activeInvoice.value.id}/status`)
            if (res.data?.success && res.data.status === 'paid') {
                activeInvoice.value.status = 'paid'
                const invInList = props.invoices.find(i => i.id === activeInvoice.value.id)
                if (invInList) invInList.status = 'paid'
                autoSuccessMsg.value = 'Ngân hàng đã nhận tiền! Hệ thống tự động gạch nợ thành công 🎉'
                stopPolling()
            }
        } catch (e) {
            // quiet poll
...
const triggerSimulatedPayment = async () => {
    ...
    try {
        ...
        if (res.data?.success) {
            ...
            const invInList = props.invoices.find(i => i.id === activeInvoice.value.id)
            if (invInList) invInList.status = 'paid'
            autoSuccessMsg.value = 'Tín hiệu Webhook Ngân hàng đã khớp! Hóa đơn tự động ĐÃ THANH TOÁN 🎉'
            stopPolling()
        }
    } catch (err) {
...
const landlordBankInfo = computed(() => {
    if (!activeInvoice.value) return { bankName: '', bankAcc: '', bankAccName: '' }
...
const transferMemo = computed(() => {
    ...
    const roomStr = rawRoom ? (rawRoom.toUpperCase().startsWith('P') ? rawRoom.replace(/[^a-zA-Z0-9]/g, '') : 'P' + rawRoom.replace(/[^a-zA-Z0-9]/g, '')) : 'PHONG'

    const rawName = activeInvoice.value.contract?.tenant?.name || props.user?.name || ''
...
<template>
    ...
    <div id="detailModal" class="modal" :style="{ display: showDetailModal ? 'flex' : 'none' }">
        <div class="modal-box" v-if="activeInvoice">
            ...
            <div class="meter-images" v-if="(elecDetail && elecDetail.meter_image_path) || (waterDetail && waterDetail.meter_image_path)">
            ...
            <div v-if="activeInvoice.status === 'paid'" class="mt-4 p-6 bg-gradient-to-br from-emerald-500 to-teal-600 text-white rounded-2xl text-center shadow-lg shadow-emerald-500/20">
                <div class="w-14 h-14 bg-white/20 rounded-full flex items-center justify-center mx-auto mb-3 text-3xl">
                    <i class="bi bi-patch-check-fill"></i>
                </div>
                <h4 class="font-black text-lg mb-1 tracking-wide">HÓA ĐƠN ĐÃ ĐƯỢC THANH TOÁN</h4>
                <p class="text-xs text-emerald-50 opacity-90">Hệ thống đã nhận được tiền và tự động gạch nợ thành công.</p>
            </div>
            ...
            <div class="thanhtoan" v-else>
                ...
                <div v-if="autoSuccessMsg" class="mb-3 p-3 bg-emerald-50 border border-emerald-300 text-emerald-800 text-xs rounded-xl text-center font-bold shadow-sm">
                    <i class="bi bi-check-circle-fill text-emerald-600 mr-1"></i> {{ autoSuccessMsg }}
                </div>

                <!-- Toast sao chép thành công -->
                <div v-if="copySuccessMsg" class="mb-3 p-2 bg-blue-50 border border-blue-200 text-blue-700 text-xs rounded-lg text-center font-bold">
                    <i class="bi bi-check-circle-fill mr-1"></i> {{ copySuccessMsg }}
                </div>

                <!-- VietQR Code Dynamic Box -->
                <div class="qr-box p-4 bg-slate-50 border border-slate-200 rounded-2xl text-center space-y-3" :style="{ display: selectedPaymentMethod === 'qr' ? 'block' : 'none' }">
                    <p class="text-xs font-bold text-slate-600">Quét mã VietQR (Tự động điền Số tiền & Nội dung)</p>
                    ...
                    <!-- Bank Account Info -->
                    <div class="mt-3 text-left bg-white p-3 rounded-xl border border-slate-200 space-y-2 text-xs font-medium text-slate-700">
                        <div class="flex justify-between items-center pb-1 border-b border-slate-100">
                            <span class="text-slate-400">Ngân hàng:</span>
                            <span class="font-bold text-slate-900">{{ landlordBankInfo.bankName }}</span>
                        </div>
                        <div class="flex justify-between items-center pb-1 border-b border-slate-100">
                            <span class="text-slate-400">Chủ tài khoản:</span>
                            <span class="font-bold text-slate-900 uppercase">{{ landlordBankInfo.bankAccName }}</span>
                        </div>
                        <div class="flex justify-between items-center pb-1 border-b border-slate-100">
                            <span class="text-slate-400">Số tài khoản:</span>
                            <div class="flex items-center gap-1.5">
                                <span class="font-bold font-mono text-blue-600 text-sm">{{ landlordBankInfo.bankAcc }}</span>
                                <button @click="copyToClipboard(landlordBankInfo.bankAcc, 'Số tài khoản')" class="px-2 py-0.5 bg-blue-50 text-blue-600 hover:bg-blue-100 rounded text-[10px] font-bold">
                        ...
                        <div class="flex justify-between items-center pb-1 border-b border-slate-100">
                            ...
                            <div class="flex items-center gap-1.5">
                                <span class="font-extrabold text-emerald-600 text-sm">{{ formatMoney(activeInvoice.total_amount) }}</span>
                                <button @click="copyToClipboard(Math.round(activeInvoice.total_amount), 'Số tiền')" class="px-2 py-0.5 bg-emerald-50 text-emerald-600 hover:bg-emerald-100 rounded text-[10px] font-bold">
                                    <i class="bi bi-copy"></i> Chép
                                </button>
                            </div>
                        </div>
                        <div class="flex justify-between items-center">
                            ...
                            <div class="flex items-center gap-1.5">
                                <span class="font-bold text-slate-800 bg-amber-50 text-amber-700 px-2 py-0.5 rounded border border-amber-200 text-xs">{{ transferMemo }}</span>
                                <button @click="copyToClipboard(transferMemo, 'Nội dung chuyển khoản')" class="px-2 py-0.5 bg-amber-100 text-amber-800 hover:bg-amber-200 rounded text-[10px] font-bold">
                                    <i class="bi bi-copy"></i> Chép
                                </button>
                            </div>
                        </div>
                    ...
                    <div class="pt-2 border-t border-slate-200">
                        <button type="button" @click="triggerSimulatedPayment" :disabled="isSimulating" class="w-full py-2.5 bg-slate-900 hover:bg-slate-800 text-white rounded-xl text-xs font-bold transition-all shadow-md flex items-center justify-center gap-2">
                            <i class="bi bi-robot text-emerald-400 text-sm"></i>
                            <span>{{ isSimulating ? 'Đang gửi tín hiệu Ngân hàng...' : '⚡ Thử nghiệm Ngân hàng báo tiền về (Demo Auto Payment)' }}</span>
                        </button>
                    </div>
                </div>

                <button class="btn-pay mt-4 w-full py-3 bg-emerald-600 hover:bg-emerald-700 text-white font-bold rounded-xl shadow-md shadow-emerald-600/20 transition-all flex items-center justify-center gap-2" @click="confirmPayment" :disabled="confirmPaymentForm.processing">
                    <i class="bi bi-send-check-fill"></i>
                    <span>{{ confirmPaymentForm.processing ? 'Đang gửi thông báo...' : 'Xác nhận đã chuyển khoản' }}</span>
                </button>
            </div>
        </div>
    </div>
