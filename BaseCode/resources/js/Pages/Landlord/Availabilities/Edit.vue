...
// List of days in the week formatted for display
...
// Helper function to initialize availabilities array for a boarding house
...
// Watch for boarding house change to reload data
...
<template>
    <LandlordLayout>
        <div class="space-y-6 max-w-5xl mx-auto pb-12">
            ...
            <div v-if="boardingHouses.length === 0" class="bg-white border border-slate-100 rounded-3xl p-8 text-center shadow-sm max-w-lg mx-auto mt-8">
                ...
                <a :href="route('landlord.rooms')" class="px-5 py-2.5 bg-emerald-500 hover:bg-emerald-600 text-white font-bold text-xs rounded-xl shadow-md shadow-emerald-500/10 transition-all inline-flex items-center gap-1.5">
            ...
            <div v-else class="grid grid-cols-1 lg:grid-cols-3 gap-6 items-start">
                ...
                <div class="lg:col-span-1 bg-white border border-slate-150 rounded-3xl p-6 shadow-sm space-y-6">
                    <div>
                        <label class="block text-xs font-bold text-slate-400 uppercase tracking-wider mb-2">Chọn cơ sở trọ</label>
                        <div class="relative">
                            <select v-model="selectedBoardingHouseId" class="w-full text-xs font-bold text-slate-700 bg-slate-50 border border-slate-200/80 rounded-xl px-4 py-3 appearance-none focus:outline-none focus:ring-2 focus:ring-emerald-500/20 focus:border-emerald-500 transition-all cursor-pointer">
                    ...
                    <div class="border-t border-slate-50 pt-5 space-y-4 text-xs font-medium text-slate-500 leading-relaxed">
                        ...
                        <ul class="list-disc pl-4 space-y-2.5">
                            <li>Bật nút gạt <span class="text-emerald-600 font-bold">Hoạt động</span> của những ngày bạn có thể dẫn khách đi xem phòng.</li>
                            <li>Thiết lập khung giờ bắt đầu và kết thúc (Ví dụ: <span class="font-bold">08:00</span> đến <span class="font-bold">18:00</span>).</li>
                            <li>Sử dụng nút <i class="bi bi-copy text-emerald-500"></i> để áp dụng nhanh giờ đó sang tất cả các ngày hoạt động khác.</li>
                            <li>Bấm <span class="font-bold text-slate-800">Lưu Cấu Hình</span> để ghi nhận các thay đổi lên hệ thống.</li>
                        </ul>
                    </div>
                </div>
                    ...
                    <form @submit.prevent="submit" class="space-y-4">
                        <div class="bg-white border border-slate-100 rounded-3xl shadow-sm overflow-hidden">
                            <div class="px-6 py-4 bg-slate-50 border-b border-slate-100 flex justify-between items-center">
                                <h3 class="text-xs font-bold text-slate-500 uppercase tracking-wider">Cài đặt lịch hàng tuần</h3>
                            ...
                            <div class="divide-y divide-slate-100">
                                <div v-for="(item, idx) in form.availabilities" :key="item.day_of_week" 
                                     :class="['p-5 transition-colors duration-200 flex flex-col sm:flex-row sm:items-center justify-between gap-4', 
                                    ...
                                    <div class="flex items-center justify-between sm:justify-start gap-4 min-w-[140px]">
                                        <div class="space-y-0.5">
                                            <span class="text-sm font-bold text-slate-800">{{ daysOfWeekList[idx].label }}</span>
                                            <p class="text-[10px] font-semibold" :class="item.is_active ? 'text-emerald-500' : 'text-slate-400'">
                                    ...
                                    <div class="w-full sm:w-auto text-xs text-rose-500 font-bold block" v-if="form.errors[`availabilities.${idx}.start_time`] || form.errors[`availabilities.${idx}.end_time`]">
                        ...
                        <div class="flex justify-end gap-3">
                            <button type="submit" :disabled="form.processing"
                                ...
                                <span v-if="form.processing" class="inline-block animate-spin border-2 border-white border-t-transparent rounded-full w-3.5 h-3.5"></span>
        ...
        <Teleport to="body">
            <Transition name="toast">
                <div v-if="toast.show" class="fixed bottom-6 right-6 z-50 flex items-center gap-3 px-4 py-3.5 rounded-2xl shadow-xl border text-xs font-bold bg-white transition-all max-w-sm"
                    ...
                    <button type="button" @click="toast.show = false" class="text-slate-400 hover:text-slate-600 transition-colors">
