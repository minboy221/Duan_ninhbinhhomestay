...
watch(() => props.show, (v) => {
    ...
    if (props.isEdit && props.room) {
        ...
        form.value = {
            room_number: props.room.name ? props.room.name.replace(/\D/g, '') : '',
...
const validate = () => {
    ...
    if (addMode.value === 'single') {
        const num = form.value.room_number ? form.value.room_number.toString().trim() : ''
        form.value.room_number = num
        if (!num) { errors.value.room_number = 'Vui lòng nhập số phòng'; return false }
        ...
        if (isDuplicate) { errors.value.room_number = `Số phòng "${name}" đã tồn tại`; return false }
    } else {
        ...
        if (multipleCount.value > 50) {
            errors.value.multipleCount = 'Chỉ có thể tạo tối đa 50 phòng cùng lúc'
            return false
        }
    }
...
const submit = () => {
    ...
    if (addMode.value === 'single') {
        ...
        const floorMatch = props.floorName ? props.floorName.match(/\d+/) : null
            ...
            <div class="p-6 space-y-4 overflow-y-auto flex-1">
                ...
                <div v-if="!isEdit" class="flex items-center bg-slate-50 p-1 rounded-xl w-fit border border-slate-100">
                    <button 
                        ...
                        :class="['px-4 py-1.5 rounded-lg text-xs font-bold transition-all', addMode === 'single' ? 'bg-white text-emerald-600 shadow-sm' : 'text-slate-400 hover:text-slate-600']"
                    >
                        Thêm 1 phòng
                    </button>
                    <button 
                        @click="addMode = 'multiple'"
                        :class="['px-4 py-1.5 rounded-lg text-xs font-bold transition-all', addMode === 'multiple' ? 'bg-white text-emerald-600 shadow-sm' : 'text-slate-400 hover:text-slate-600']"
                    >
                        Thêm nhiều phòng
                    </button>
                </div>
                ...
                <div v-if="addMode === 'single'" class="space-y-1">
                    <label class="text-xs font-bold text-slate-500">Số phòng <span v-if="!isEdit" class="text-rose-500">*</span></label>
                    
                    <div v-if="isEdit" class="w-full px-3.5 py-2.5 border border-slate-200 bg-slate-50 rounded-xl text-xs font-bold text-slate-500 cursor-not-allowed">
                        P.{{ form.room_number }}
                    </div>
                ...
                <div v-if="addMode === 'multiple'" class="space-y-1">
                    <label class="text-xs font-bold text-slate-500">Số lượng phòng cần tạo <span class="text-rose-500">*</span></label>
                    <input 
                        ...
                        :class="[
                            'w-full px-3.5 py-2.5 border rounded-xl text-xs font-medium outline-none transition-all',
                            errors.multipleCount ? 'border-rose-300 bg-rose-50/50 focus:border-rose-500' : 'border-slate-200 focus:border-emerald-500'
                        ]" 
                        @input="errors.multipleCount=''"
                    />
                    <span v-if="errors.multipleCount" class="text-[10px] text-rose-500 font-semibold flex items-center gap-1 mt-1">
                        <i class="bi bi-exclamation-circle"></i> {{ errors.multipleCount }}
                    </span>
                ...
                <div v-if="form.status === 'maintenance'" class="space-y-1">
                    <label class="text-xs font-bold text-slate-500">Lý do bảo trì <span class="text-rose-500">*</span></label>
                    <input 
                        v-model="form.maintenance_reason" 
                        :class="[
                            'w-full px-3.5 py-2.5 border rounded-xl text-xs font-medium outline-none transition-all',
                            errors.maintenance_reason ? 'border-rose-300 bg-rose-50/50 focus:border-rose-500' : 'border-slate-200 focus:border-emerald-500'
                        ]" 
                        placeholder="VD: Sửa ống nước, sơn lại tường..." 
                        @input="errors.maintenance_reason=''"
                    />
                    <span v-if="errors.maintenance_reason" class="text-[10px] text-rose-500 font-semibold flex items-center gap-1 mt-1">
                        <i class="bi bi-exclamation-circle"></i> {{ errors.maintenance_reason }}
                    </span>
                </div>
            </div>
