...
const statusConfig = {
    ...
    pending:    { label: 'Chờ Duyệt',     cls: 'vs-pending',    icon: 'bi-hourglass-split',   desc: 'Hồ sơ đang được Admin xem xét. Thường 1-2 ngày làm việc.' },
    verified:   { label: 'Đã Xác Minh',   cls: 'vs-verified',   icon: 'bi-patch-check-fill',  desc: 'Tài khoản đã được xác minh. Bạn có thể đăng tin thoải mái.' },
}
</script>

<template>
    <LandlordLayout>
        <template #header-title><h1 class="ll-header-title">Thông Tin Chủ Trọ</h1></template>
        ...
        <div class="prof-wrap">
            ...
            <div :class="['verify-banner', statusConfig[verifyStatus].cls]">
                ...
                <div class="vb-text">
                    <span class="vb-label">Trạng thái: <strong>{{ statusConfig[verifyStatus].label }}</strong></span>
                    <span class="vb-desc">{{ statusConfig[verifyStatus].desc }}</span>
                </div>
            </div>
...
<style scoped>
.prof-wrap { display: flex; flex-direction: column; gap: 20px; }
...
.vb-text { display: flex; flex-direction: column; gap: 2px; }
.vb-label { font-weight: 600; }
.vb-desc  { font-size: 12px; opacity: 0.85; }

.vs-unverified { background: #fef2f2; color: #b91c1c; border: 1.5px solid #fecaca; }
.vs-pending    { background: #fffbeb; color: #92400e; border: 1.5px solid #fcd34d; }
.vs-verified   { background: #f0fdf4; color: #065f46; border: 1.5px solid #6ee7b7; }
...
.avatar-box { width: 90px; height: 90px; border-radius: 50%; overflow: hidden; border: 3px solid #6ee7b7; }
.avatar-img { width: 100%; height: 100%; object-fit: cover; }
.avatar-placeholder { width: 100%; height: 100%; background: #0f766e; display: flex; align-items: center; justify-content: center; color: #fff; font-size: 36px; }
.avatar-change {
...
/* Docs */
.sec-title { font-size: 15px; font-weight: 700; color: #064e3b; margin: 0; display: flex; align-items: center; gap: 7px; }
.sec-desc  { font-size: 13px; color: #6b7280; margin: 0; }

.doc-section { display: flex; flex-direction: column; gap: 8px; padding-bottom: 14px; border-bottom: 1px solid #f0fdf4; }
.doc-section:last-of-type { border-bottom: none; }
.doc-label { font-size: 12px; font-weight: 700; color: #374151; }
...
.doc-upload:hover { border-color: #0f766e; }
.doc-uploaded { border-color: #0f766e !important; border-style: solid !important; }
.doc-portrait { height: 120px; max-width: 150px; }
.doc-wide { width: 100%; height: 100px; }

.doc-placeholder { display: flex; flex-direction: column; align-items: center; gap: 6px; color: #9ca3af; }
.doc-placeholder i { font-size: 28px; color: #6ee7b7; }
.doc-placeholder span { font-size: 12px; text-align: center; }
