...
<template>
    <div class="max-w-5xl mx-auto">
        ...
        <div class="text-center mb-12">
            <h1
                class="text-4xl font-extrabold text-on-surface tracking-tight mb-3"
            >
        ...
        <div class="grid grid-cols-1 lg:grid-cols-12 gap-8">
            <div class="lg:col-span-7 space-y-8">
                <!-- Basic Info Card -->
                <section
                    class="glass-card p-8 rounded-xl shadow-sm relative z-20"
                >
                    <h2
                        class="text-xl font-bold text-on-surface mb-6 flex items-center gap-2"
                    >
                    ...
                    <div class="space-y-6">
                        ...
                        <div class="space-y-1">
                            <label
                                class="block text-sm font-semibold text-on-surface mb-2 tracking-wide uppercase"
                                >Tên Homestay
                                    ...
                                    <transition name="dropdown-fade">
                                        ...
                                        >
                                            ...
                                            >
                                                <span>{{ commune }}</span>
                                                <span
                                                    v-if="form.ward === commune"
                                                    class="material-symbols-outlined text-sm text-primary font-extrabold"
                                                    >check</span
                                                >
                                            </button>
                                        </div>
                ...
                <section
                    class="glass-card p-8 rounded-xl shadow-sm space-y-4 relative z-10"
                >
                    <h2
                        class="text-xl font-bold text-on-surface mb-2 flex items-center gap-2"
                    >
                        <span class="material-symbols-outlined text-primary"
                            >description</span
                        >
                    ...
                    <label class="relative block group">
                        ...
                        >
                            <span
                                class="material-symbols-outlined text-4xl text-outline mb-4"
                                >cloud_upload</span
                            >
                    ...
                    >
                        ...
                        >
                            ...
                            <div class="flex-1 min-w-0">
                                <p
                                    class="text-sm font-medium text-on-surface truncate"
                                    :title="file.name"
                                >
                                    {{ file.name }}
                                </p>
                            ...
                            <button
                                type="button"
                                @click="removeFile(index, 'contract_images')"
            ...
            <div class="lg:col-span-5 space-y-8">
                <!-- Gallery Section -->
                <section
                    class="glass-card p-8 rounded-xl h-full flex flex-col justify-between"
                >
                    <div>
                        <h2
                            class="text-xl font-bold text-on-surface mb-6 flex items-center gap-2"
                        ...
                        <label class="relative block group mb-6">
                            ...
                            >
                                <span
                                    class="material-symbols-outlined text-2xl text-outline mb-2"
                                    >video_camera_back</span
                                >
                                <p class="text-sm font-medium">
                                    Tải lên Ảnh / Video Homestay
                                </p>
                                <p
                                    class="text-xs text-outline font-normal mt-1"
                                >
                                    (Hỗ trợ JPG, PNG, HEIC, MP4, MOV - Tối đa
                                    20MB)
                                </p>
                            </div>
                            <input
                        ...
                        <p
                            ...
                            class="text-error text-xs font-bold mb-4"
                        >
                            {{ errors.room_images }}
                        </p>
                        ...
                        <div
                            ...
                            class="grid grid-cols-2 gap-3 h-64"
                        >
                            <!-- Image 1 (Left / Large) -->
                            <div
                                v-if="form.room_images_preview[0]"
                            ...
                            >
                                <img
                                    ...
                                    "
                                    :src="form.room_images_preview[0].url"
                                ...
                                >
                                    <button
                                        type="button"
                                        @click="removeFile(0, 'room_images')"
                            ...
                            <div class="grid grid-rows-2 gap-3">
                                <!-- Image 2 -->
                                <div
                                    v-if="form.room_images_preview[1]"
                                    class="relative rounded-lg overflow-hidden group border border-outline-variant/20 bg-slate-950"
                                >
                                    <img
                                        v-if="
                                        ...
                                        "
                                        :src="form.room_images_preview[1].url"
                                        class="w-full h-full object-cover"
                                    />
                                    <video
                                        v-else
                                ...
                                <div
                                    v-if="form.room_images_preview[2]"
                                    class="relative rounded-lg overflow-hidden group border border-outline-variant/20 bg-slate-950"
                                >
                                    <img
                                        v-if="
                                        ...
                                        "
                                        :src="form.room_images_preview[2].url"
                                        class="w-full h-full object-cover"
                                    />
                                    <video
                                        v-else
                        ...
                        <div
                            v-if="form.room_images_preview?.length > 3"
                        ...
                        >
                            <div
                                v-for="(
                                    file, index
                                ) in form.room_images_preview.slice(3)"
                                :key="'room-extra-' + index"
                                class="flex items-center justify-between text-xs p-1.5 hover:bg-surface-container rounded transition-colors"
                            >
                                <span
                                    class="truncate max-w-[200px] font-medium text-on-surface-variant"
                                    >{{ file.name }}</span
                                >
                                <button
                                    type="button"
                    ...
                    <p class="text-xs text-on-surface-variant italic mt-3">
                        * Tọa độ sẽ được trích xuất từ dữ liệu GPS nằm trong ảnh
                        đã tải lên để hiển thị chính xác trong admin.
                    </p>
                </section>
            </div>
        </div>
        ...
        <!-- Navigation Buttons -->
        <div
            class="mt-12 flex flex-col-reverse md:flex-row justify-between items-center gap-4 w-full"
        >
            <button
                type="button"
                @click="emit('prev')"
