                ...
                <form @submit.prevent="submit" class="space-y-6">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        ...
                        <div class="col-span-1 md:col-span-2">
                            <label class="block text-sm font-bold text-slate-700 mb-2">Tên Cơ Sở / Khu Trọ <span
                            ...
                            <input v-model="form.name" type="text"
                                ...
                                placeholder="VD: Trọ Sinh Viên Cầu Giấy" required>
                            <div v-if="form.errors.name" class="text-rose-500 text-xs mt-1">{{ form.errors.name }}</div>
                        </div>

                        <!-- Khu vực -->
                        <div>
                            <label class="block text-sm font-bold text-slate-700 mb-2">Quận / Huyện / Xã <span
                                    class="text-rose-500">*</span></label>
                            <input v-model="form.district" type="text"
                                class="w-full bg-slate-50 border border-slate-200 rounded-xl px-4 py-3 text-sm focus:ring-2 focus:ring-emerald-500/20 focus:border-emerald-500 transition-all outline-none"
                                placeholder="Nhập khu vực..." required>
                            <div v-if="form.errors.district" class="text-rose-500 text-xs mt-1">{{ form.errors.district
                                }}</div>
                        </div>

                        <!-- Địa chỉ chi tiết -->
                        <div>
                            <label class="block text-sm font-bold text-slate-700 mb-2">Địa Chỉ Chi Tiết <span
                                    class="text-rose-500">*</span></label>
                            ...
                            <div v-if="form.errors.address_detail" class="text-rose-500 text-xs mt-1">{{
                                form.errors.address_detail }}</div>
                        </div>

                        <!-- Chỉ dẫn đường đi (Cẩm nang ngõ ngách) -->
                        <div class="col-span-1 md:col-span-2">
                            <label class="block text-sm font-bold text-slate-700 mb-2">Chỉ dẫn đường đi (Cẩm nang tìm phòng) <span class="text-slate-400 font-normal">(Không bắt buộc)</span></label>
                            ...
                            <div v-if="form.errors.directions_guide" class="text-rose-500 text-xs mt-1">{{
                                form.errors.directions_guide }}</div>
                        </div>
                        ...
                        <div class="col-span-1 md:col-span-2 bg-emerald-50/50 p-4 rounded-xl border border-emerald-100">
                            <div class="flex items-center justify-between mb-4">
                                <div>
                                    <h3 class="text-sm font-bold text-emerald-800">Tọa độ Bản Đồ (GPS) <span
                                            class="text-rose-500">*</span></h3>
                                    <p class="text-xs text-emerald-600/80 mt-0.5">Giúp khách hàng dễ dàng tìm thấy nhà
                                        trọ của bạn trên bản đồ</p>
                                </div>
                            ...
                            <div v-if="form.errors.latitude || form.errors.longitude"
                                class="text-rose-500 text-xs mt-2">Vui lòng nhập đầy đủ tọa độ GPS</div>
                        ...
                        <div class="col-span-1 md:col-span-2">
                            <label class="block text-sm font-bold text-slate-700 mb-2">Ảnh chụp khu trọ / Mặt tiền <span
                                    class="text-rose-500">*</span></label>
                            <div class="relative w-full">
                                <input type="file" multiple accept="image/*" @change="e => handleFileChange(e, 'room')"
                    ...
                    <div class="flex justify-end gap-3">
                        <Link href="/landlord/boarding-houses"
                        ...
                        <button type="submit" :disabled="form.processing"
                            ...
                            <span v-if="form.processing"><i class="bi bi-arrow-repeat animate-spin"></i> Đang Xử
                                Lý...</span>
