...
const form = useForm({
    _method: "POST", // Giả lập phương thức POST để Laravel nhận diện tải file lên
    name: props.house.name || "",
    district: props.house.district || "",
    address_detail: props.house.address_detail || "",
    directions_guide: props.house.directions_guide || "",
...
// Nạp lại ảnh cũ của cơ sở trọ hiển thị lên khung preview
onMounted(() => {
    if (props.house.room_images) {
        try {
            const arr =
...
const getLocation = () => {
    if (!navigator.geolocation) {
        showWarning("Lỗi trình duyệt không hỗ trợ Geolocation.");
        return;
    }
    navigator.geolocation.getCurrentPosition(
        ...
        (erorr) => {
            showWarning(
                "Lỗi GPS",
                "Không thể lấy vị trí.Vui lòng cập nhật thủ công.",
            );
        },
...
const submit = () => {
    form.post(route("landlord.boarding-houses.update", props.house.id), {
        preserveScroll: true,
        onError: (err) => {
            showError("Có lỗi xảy ra", "Vui lòng kiểm tra lại thông tin nhập.");
        },
    });
};
        ...
        <div class="max-w-4xl mx-auto">
            <div class="mb-6">
                ...
                <p class="text-sm text-slate-500 mt-1">
                    Cập nhật thông tin chi tiết khu trọ của bạn
                </p>
            </div>
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        ...
                        <div class="col-span-1 md:col-span-2">
                            ...
                            <textarea v-model="form.directions_guide" rows="3"
                                ...
                                placeholder="VD: Đi vào ngõ 12, rẽ phải ở cây cột điện thứ 2..."></textarea>
                            ...
                            <div class="flex items-center justify-between mb-4">
                                <div>
                                    ...
                                    <p class="text-xs text-emerald-600/80 mt-0.5">
                                        Xác định tọa độ vị trí cơ sở trọ của bạn
                                        trên bản đồ
                                    </p>
                                </div>
                        ...
                        <!-- Ảnh nhà / Ảnh mặt tiền (Không bắt buộc khi sửa) -->
                        <div class="col-span-1 md:col-span-2">
                            <label class="block text-sm font-bold text-slate-700 mb-2">Ảnh chụp khu trọ / Mặt tiền
                                <span class="text-slate-400 font-normal">(Chỉ chọn nếu muốn thay đổi ảnh
                                    cũ)</span></label>
                            <div class="relative w-full">
                                ...
                                <div
                                    ...
                                    <span class="text-sm font-bold text-slate-600">Bấm hoặc kéo thả ảnh mới vào
                                        đây</span>
                    ...
                    <div class="flex justify-end gap-3">
                        ...
                        <button type="submit" :disabled="form.processing"
                            ...
                            <span v-if="form.processing"><i class="bi bi-arrow-repeat animate-spin"></i>
                                Đang Lưu...</span>
                            <span v-else><i class="bi bi-check-lg"></i> Lưu Thay
                                Đổi</span>
                        </button>
                    </div>
                </form>
