    ...
    <LandlordLayout>
        <div class="space-y-6">
            ...
            <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
                <div class="space-y-1">
                    <h2 class="text-lg font-bold text-slate-800">Lịch hẹn khách hàng</h2>
                    <p class="text-xs text-slate-400">Xem và sắp xếp thời gian dẫn khách đi xem thực tế căn hộ/phòng trọ</p>
                </div>
            </div>

            <!-- Summary Chips -->
            <div class="flex flex-wrap items-center gap-3">
                <div class="px-4 py-2 border border-slate-200 bg-white rounded-xl text-xs font-bold text-slate-600 flex items-center gap-1.5 shadow-sm">
                    <i class="bi bi-calendar-event text-emerald-500"></i> Tổng lịch: {{ appointments.length }}
                </div>
                <div class="px-4 py-2 border border-amber-200 bg-amber-50/10 rounded-xl text-xs font-bold text-amber-600 flex items-center gap-1.5 shadow-sm">
                    <i class="bi bi-hourglass-split"></i> Chờ duyệt: {{ pendingList.length }}
                </div>
                <div class="px-4 py-2 border border-emerald-250 bg-emerald-50/10 rounded-xl text-xs font-bold text-emerald-600 flex items-center gap-1.5 shadow-sm">
                    <i class="bi bi-check-circle-fill"></i> Đã duyệt: {{ appointments.filter(a=>a.status==='approved').length }}
                </div>
            </div>
            ...
            <div class="grid grid-cols-1 lg:grid-cols-12 gap-6 items-start">
                ...
                <div class="bg-white border border-slate-100 rounded-3xl p-5 shadow-sm lg:col-span-4 space-y-4">
                    ...
                    <div class="grid grid-cols-7 gap-1.5 text-xs font-semibold text-slate-600">
                        <div 
                            ...
                            :class="[
                                ...
                                day ? 'bg-slate-50/50 hover:bg-slate-100/60 cursor-pointer' : '',
                                aptsForDay(day).length > 0 ? 'bg-emerald-50/30 border-emerald-200' : ''
                            ]"
                        >
                            ...
                            <div v-if="day" class="flex gap-0.5 justify-center">
                                <span 
                                    ...
                                    :class="[
                                        ...
                                        apt.status === 'pending' ? 'bg-amber-400' : apt.status === 'approved' ? 'bg-emerald-500' : 'bg-rose-400'
                                    ]"
                                ></span>
                            </div>
                        </div>
                    ...
                    <!-- Calendar Legend -->
                    <div class="border-t border-slate-50 pt-3 flex justify-between text-[10px] font-bold text-slate-400">
                        <span class="flex items-center gap-1"><span class="w-2 h-2 rounded-full bg-amber-400"></span> Chờ duyệt</span>
                        <span class="flex items-center gap-1"><span class="w-2 h-2 rounded-full bg-emerald-500"></span> Đã duyệt</span>
                        <span class="flex items-center gap-1"><span class="w-2 h-2 rounded-full bg-rose-500"></span> Từ chối</span>
                    </div>
                </div>
                ...
                <div class="lg:col-span-8 space-y-6">
                    ...
                    <div class="bg-white border border-slate-100 rounded-3xl p-6 shadow-sm space-y-4">
                        <h3 class="text-sm font-bold text-slate-800 flex items-center gap-2">
                            ...
                            <span>Yêu cầu xem phòng chờ duyệt ({{ pendingList.length }})</span>
                        </h3>

                        <div v-if="pendingList.length === 0" class="p-6 text-center text-xs text-slate-400 font-semibold bg-slate-50/50 rounded-2xl border border-slate-100">
                            <i class="bi bi-check2-all text-2xl text-emerald-500 block mb-1"></i>
                            Tuyệt vời! Không còn lịch hẹn nào chưa xử lý.
                        </div>
                        <div v-else class="space-y-3">
                            <div 
                                v-for="apt in pendingList" 
                                :key="apt.id"
                                :class="[
                                    'p-4 border rounded-2xl flex flex-col md:flex-row justify-between md:items-center gap-4 transition-all',
                                    hasConflict(apt) ? 'bg-rose-50/20 border-rose-200' : 'border-slate-100'
                                ]"
                            >
                                <div class="space-y-2">
                                    <!-- Warning badge if conflict exists -->
                                    <div v-if="hasConflict(apt)" class="px-2 py-0.5 bg-rose-50 text-rose-600 border border-rose-100 rounded text-[9px] font-bold w-fit flex items-center gap-1">
                                        <i class="bi bi-exclamation-triangle-fill"></i> Trùng lịch xem phòng khác!
                                    </div>

                                    <div class="space-y-0.5">
                                        <h4 class="text-xs font-bold text-slate-800">{{ apt.name }} <span class="text-slate-400 font-semibold">({{ apt.phone }})</span></h4>
                                        <div class="flex flex-wrap gap-x-3 gap-y-1 text-xs text-slate-400 font-semibold">
                                            <span><i class="bi bi-house"></i> {{ apt.room }}</span>
                                            <span><i class="bi bi-clock"></i> {{ apt.time }} · {{ new Date(apt.date).toLocaleDateString('vi-VN') }}</span>
                                        </div>
                                    </div>
                                    <p v-if="apt.note" class="text-xs text-slate-400 font-semibold bg-slate-50 p-2 rounded-xl border border-slate-100">
                                        <i class="bi bi-chat-left-text"></i> Ghi chú: {{ apt.note }}
                                    </p>
                                </div>

                                <div class="flex items-center gap-2">
                                    <button @click="approveApt(apt)" class="px-4 py-2 bg-emerald-50 hover:bg-emerald-100 text-emerald-600 font-bold text-xs rounded-xl transition-colors flex items-center gap-1">
                                        <i class="bi bi-check-lg"></i> Duyệt
                                    </button>
                                    <button @click="rejectApt(apt)" class="px-3.5 py-2 hover:bg-rose-50 text-rose-500 rounded-xl transition-colors"><i class="bi bi-x-lg"></i></button>
                                </div>
                            </div>
                        </div>
                    </div>
                                    ...
                                    <tr v-for="apt in appointments" :key="apt.id" class="hover:bg-slate-50/30">
                                        <td class="py-3 px-4">
                                            <div class="flex flex-col">
                                                <span>{{ apt.name }}</span>
                                                <span class="text-[10px] text-slate-400 font-semibold">{{ apt.phone }}</span>
                                            </div>
                                        </td>
                                        <td class="py-3 px-4 font-bold text-emerald-600">{{ apt.room }}</td>
                                        <td class="py-3 px-4 text-slate-500">{{ apt.time }} · {{ new Date(apt.date).toLocaleDateString('vi-VN') }}</td>
                                        <td class="py-3 px-4">
                                            <span :class="['px-2.5 py-1 rounded-md text-[10px] font-bold border flex items-center gap-1.5 w-fit', statusMap[apt.status].cls]">
