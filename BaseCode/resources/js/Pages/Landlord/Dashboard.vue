        ...
        <div class="space-y-8 pb-12">
            ...
            <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
                ...
                <div class="lg:col-span-2 bg-gradient-to-tr from-slate-950 via-[#064e3b] to-emerald-950 text-white rounded-3xl p-8 relative overflow-hidden flex flex-col justify-between min-h-[180px] shadow-lg shadow-emerald-950/10">
                    ...
                    <div class="z-10 mt-6">
                        <Link href="/landlord/invoices" class="px-6 py-3 bg-white text-emerald-900 font-extrabold text-sm rounded-xl shadow-lg hover:shadow-white/10 hover:-translate-y-0.5 active:scale-95 transition-all inline-block">
                            Tạo hóa đơn ngay
                        </Link>
                    </div>
                </div>
                ...
                <div class="bg-white border border-slate-100 rounded-3xl p-8 flex flex-col justify-between shadow-[0_8px_30px_rgb(0,0,0,0.015)] relative overflow-hidden group">
                    ...
                    <div class="space-y-3">
                        <div class="flex items-center gap-2">
                            <span class="px-2 py-0.5 text-[10px] font-extrabold text-amber-700 bg-amber-50 border border-amber-200/50 rounded-md uppercase tracking-wider">PRO</span>
                            <span class="text-sm font-extrabold text-slate-800 uppercase tracking-wider">Phân quyền dễ dàng</span>
                        </div>
                    ...
                    <button class="w-full mt-6 py-3 bg-slate-50 border border-slate-200/80 hover:bg-slate-100/80 text-slate-700 font-extrabold text-sm rounded-xl transition-all duration-300">
                        Trải nghiệm Premium
                    </button>
                </div>
            </div>
            ...
            <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
                ...
                <div class="bg-white border border-slate-100 rounded-3xl p-6 flex items-center justify-between shadow-[0_8px_30px_rgb(0,0,0,0.01)] hover:-translate-y-1 hover:shadow-md hover:border-slate-200/60 transition-all duration-300">
                    <div class="space-y-3">
                        <p class="text-xs font-extrabold text-slate-400 uppercase tracking-widest">Doanh thu tháng gần nhất</p>
                ...
                <div class="bg-white border border-slate-100 rounded-3xl p-6 flex items-center justify-between shadow-[0_8px_30px_rgb(0,0,0,0.01)] hover:-translate-y-1 hover:shadow-md hover:border-slate-200/60 transition-all duration-300">
                    <div class="space-y-3">
                        <p class="text-xs font-extrabold text-slate-400 uppercase tracking-widest">Tổng khách hàng</p>
                ...
                <div class="bg-white border border-slate-100 rounded-3xl p-6 flex items-center justify-between shadow-[0_8px_30px_rgb(0,0,0,0.01)] hover:-translate-y-1 hover:shadow-md hover:border-slate-200/60 transition-all duration-300">
                    <div class="space-y-3">
                        <p class="text-xs font-extrabold text-slate-400 uppercase tracking-widest">Tổng doanh thu</p>
            ...
            <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
                ...
                <div class="lg:col-span-2 bg-white border border-slate-100 rounded-3xl p-6 shadow-[0_8px_30px_rgb(0,0,0,0.01)] flex flex-col justify-between">
                    ...
                    <div class="flex flex-wrap gap-x-4 gap-y-2 border-t border-slate-50 pt-4">
                        <div v-for="srv in servicesRevenue" :key="srv.name" class="flex items-center gap-1.5 text-sm font-bold">
            ...
            <div class="bg-white border border-slate-100 rounded-3xl p-6 shadow-[0_8px_30px_rgb(0,0,0,0.015)] overflow-hidden">
                <div class="space-y-1 mb-6">
                    ...
                    <p class="text-xs text-slate-400 font-medium">Chi tiết phân bổ doanh thu hóa đơn thực tế theo tháng</p>
                </div>
                <div class="overflow-x-auto">
                    <table class="w-full text-left border-collapse min-w-[800px]">
                        <thead>
                            <tr class="border-b border-slate-100 text-xs font-extrabold text-slate-400 uppercase tracking-widest">
            ...
            <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
                ...
                <div class="bg-white border border-slate-100 rounded-3xl p-6 shadow-[0_8px_30px_rgb(0,0,0,0.01)] flex flex-col justify-between">
                    <div>
                        <div class="flex items-center justify-between mb-5">
                            <h3 class="text-sm font-extrabold text-slate-900 uppercase tracking-wider">Hợp đồng mới</h3>
                            <Link href="/landlord/contracts" class="text-sm font-extrabold text-emerald-600 hover:text-emerald-700 transition-colors">Xem thêm</Link>
                        </div>
                        <div class="space-y-3.5">
                            <div v-for="item in recentContracts" :key="item.id" class="flex items-center justify-between p-3.5 bg-slate-50/60 border border-slate-100 rounded-2xl hover:bg-slate-50 hover:border-slate-200/80 transition-all duration-300">
                                <div class="space-y-1">
                                    <p class="text-sm font-black text-slate-800">{{ item.tenant }}</p>
                                    <p class="text-xs text-slate-400 font-extrabold">{{ item.room }} &middot; Ký ngày: {{ item.date }}</p>
                                </div>
                                <span :class="[
                ...
                <div class="bg-white border border-slate-100 rounded-3xl p-6 shadow-[0_8px_30px_rgb(0,0,0,0.01)] flex flex-col justify-between">
                    <div>
                        <div class="flex items-center justify-between mb-5">
                            <h3 class="text-sm font-extrabold text-slate-900 uppercase tracking-wider">Hóa đơn mới</h3>
                            <Link href="/landlord/invoices" class="text-sm font-extrabold text-emerald-600 hover:text-emerald-700 transition-colors">Xem thêm</Link>
                        </div>
                        <div class="space-y-3.5">
                            <div v-for="item in recentInvoices" :key="item.id" class="flex items-center justify-between p-3.5 bg-slate-50/60 border border-slate-100 rounded-2xl hover:bg-slate-50 hover:border-slate-200/80 transition-all duration-300">
                                <div class="space-y-1">
                                    <p class="text-sm font-black text-slate-800">{{ item.code }}</p>
                                    <p class="text-xs text-slate-400 font-extrabold">{{ item.room }} &middot; {{ formatMoney(item.amount) }}</p>
                                </div>
                                <span :class="[
                                    'px-2.5 py-0.5 rounded-lg text-[11px] font-extrabold uppercase tracking-wider border',
                ...
                <div class="bg-white border border-slate-100 rounded-3xl p-6 shadow-[0_8px_30px_rgb(0,0,0,0.01)] flex flex-col justify-between">
                    <div>
                        <div class="flex items-center justify-between mb-5">
                            <h3 class="text-sm font-extrabold text-slate-900 uppercase tracking-wider">Khách hàng mới</h3>
                            <Link href="/landlord/tenants" class="text-sm font-extrabold text-emerald-600 hover:text-emerald-700 transition-colors">Xem thêm</Link>
                        </div>
                        <div class="space-y-3.5">
                            <div v-for="item in recentTenants" :key="item.id" class="flex items-center justify-between p-3.5 bg-slate-50/60 border border-slate-100 rounded-2xl hover:bg-slate-50 hover:border-slate-200/80 transition-all duration-300">
                                <div class="space-y-2 flex-1">
                                    <div class="flex items-center justify-between">
                                        ...
                                        <span class="text-[11px] text-emerald-700 font-extrabold bg-emerald-50 border border-emerald-100/60 px-2 py-0.5 rounded-lg uppercase tracking-wide">{{ item.room }}</span>
                                    </div>
                                    <p class="text-xs text-slate-400 font-extrabold">SĐT: {{ item.phone }} &middot; Thuê từ: {{ item.date }}</p>
                                </div>
                            </div>
                        </div>
                    </div>
