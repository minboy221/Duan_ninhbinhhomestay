...
<template>
    ...
    <AdminLayout>
        ...
        <div class="stats-row">
            <div class="scard"><i class="bi bi-star-fill" style="color:#f59e0b"></i><div><p class="snum">{{ reviews.length }}</p><p class="slbl">Tổng đánh giá</p></div></div>
            <div class="scard"><i class="bi bi-eye-fill" style="color:#3b82f6"></i><div><p class="snum">{{ reviews.filter(r=>r.visible).length }}</p><p class="slbl">Đang hiển thị</p></div></div>
            <div class="scard"><i class="bi bi-eye-slash-fill" style="color:#94a3b8"></i><div><p class="snum">{{ reviews.filter(r=>!r.visible).length }}</p><p class="slbl">Đã ẩn</p></div></div>
...
.scard i{font-size:26px}.snum{font-size:24px;font-weight:800;color:#0f172a;margin:0;line-height:1}.slbl{font-size:11px;color:#94a3b8;margin:0}
.reviews-list{display:flex;flex-direction:column;gap:12px}
.review-card{background:#fff;border-radius:14px;border:1px solid #f1f5f9;padding:18px;display:flex;gap:16px;align-items:flex-start;box-shadow:0 1px 3px rgba(0,0,0,.04)}
.review-card:hover{box-shadow:0 4px 16px rgba(0,0,0,.08)}
.review-hidden{opacity:.5;border-style:dashed}
.rv-left{display:flex;gap:10px;flex-shrink:0;width:200px}
.rv-ava{width:36px;height:36px;border-radius:10px;background:linear-gradient(135deg,#7c3aed,#4f46e5);color:#fff;font-size:15px;font-weight:700;display:flex;align-items:center;justify-content:center;flex-shrink:0}
.rv-name{font-size:13px;font-weight:700;color:#0f172a;margin:0}
.rv-room,.rv-date{font-size:11px;color:#94a3b8;margin:2px 0 0}
.rv-content{flex:1}
.rv-stars{font-size:18px;display:flex;align-items:center;gap:6px;margin-bottom:6px}
.star-num{font-size:12px;font-weight:700;color:#0f172a}
.neg-badge{font-size:11px;background:#fef2f2;color:#ef4444;padding:2px 8px;border-radius:99px;font-weight:600}
.rv-text{font-size:13px;color:#334155;line-height:1.5;margin:0}
.rv-actions{display:flex;flex-direction:column;gap:6px;flex-shrink:0}
.act-btn{padding:7px 12px;border-radius:9px;border:none;font-size:12px;font-weight:600;cursor:pointer;display:flex;align-items:center;gap:5px}
