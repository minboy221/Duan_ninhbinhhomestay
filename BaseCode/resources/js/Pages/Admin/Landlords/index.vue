...
import { getAvatarUrl, getImageUrl, DEFAULT_AVATAR } from "@/Utils/media";
...
<template>
    ...
    <AdminLayout>
                ...
                <tbody>
                    ...
                    <tr v-for="(l, i) in landlords" :key="l.id" class="trow">
                        ...
                        <td>
                            <div class="user-cell">
                                <div class="ava"
                                    :style="l.avatar ? 'overflow: hidden; background: #f1f5f9; padding: 0;' : `background:hsl(${l.id * 80}deg,60%,55%)`">
                                    <img v-if="l.avatar" :src="getAvatarUrl(l.avatar)"
                        ...
                        <td>
                            <span class="font-bold text-slate-700">{{ l.boarding_houses_count || 0 }} cơ sở</span>
                        </td>
                        <td>
        ...
        <Teleport to="body">
                ...
                <div class="modal-box modal-lg">
                    ...
                    <div class="modal-body" style="max-height: 80vh; overflow-y: auto;">
                        ...
                        <div class="info-block">
                            ...
                            <div class="ib-row">
                                <span class="ib-l">Số cơ sở</span>
                                <span class="ib-v font-bold text-slate-800">{{ selected?.boarding_houses_count || 0 }} cơ sở</span>
                            </div>
                            <div class="ib-row">
                                <span class="ib-l">Các cơ sở trọ</span>
                                <span class="ib-v">{{ selected?.boarding_house_name }}</span>
                            </div>
                            ...
                            <div class="ib-row">
                                <span class="ib-l">Tổng số phòng</span>
                                <span class="ib-v font-bold text-emerald-600">{{ selected?.rooms || 0 }} phòng</span>
                            </div>
                            <div class="ib-row">
                        ...
                        <div class="doc-section">
                            <div class="doc-header-wrap">
                                <h4 class="doc-title">
                                    <i class="bi bi-shield-lock-fill text-purple-600"></i> Giấy Tờ Xác Minh Danh Tính
                                    (KYC)
                                </h4>
                                <button type="button" @click="toggleKycLock"
                                    :class="['btn-kyc-toggle', isKycUnlocked ? 'btn-unlocked' : 'btn-locked']">
                                    <i :class="['bi', isKycUnlocked ? 'bi-eye-slash-fill' : 'bi-eye-fill']"></i>
                                    <span>{{ isKycUnlocked ? `Khóa lại (${kycCountdown}s)` : 'Mở xem thông tin nhạy cảm'
                                        }}</span>
                                </button>
                            </div>
                            ...
                            <div class="doc-grid"
                                v-if="getPrivateImageUrl(selected?.verification?.id_card_front, 'id_cards') || getPrivateImageUrl(selected?.verification?.id_card_back, 'id_cards') || getPrivateImageUrl(selected?.verification?.face_auth_image, 'faces')">
                                <!-- CCCD Mặt trước -->
                                <div class="doc-card"
                                    v-if="getPrivateImageUrl(selected?.verification?.id_card_front, 'id_cards')">
                                    <span class="doc-lbl">CCCD Mặt trước</span>
                                    <div :class="['doc-img-wrap', !isKycUnlocked ? 'kyc-blurred' : '']"
                                        @contextmenu.prevent @dragstart.prevent
                                        @click="isKycUnlocked && openImage(getPrivateImageUrl(selected.verification.id_card_front, 'id_cards'))">
                                        <img :src="getPrivateImageUrl(selected.verification.id_card_front, 'id_cards')"
                                            alt="CCCD Trước" />
                                        ...
                                        <div class="img-overlay" v-if="isKycUnlocked"><i class="bi bi-zoom-in"></i> Xem
                                            ảnh
                                        </div>
                                    </div>
                                </div>
                                ...
                                <!-- CCCD Mặt sau -->
                                <div class="doc-card"
                                    v-if="getPrivateImageUrl(selected?.verification?.id_card_back, 'id_cards')">
                                    <span class="doc-lbl">CCCD Mặt sau</span>
                                    <div :class="['doc-img-wrap', !isKycUnlocked ? 'kyc-blurred' : '']"
                                        @contextmenu.prevent @dragstart.prevent
                                        @click="isKycUnlocked && openImage(getPrivateImageUrl(selected.verification.id_card_back, 'id_cards'))">
                                        <img :src="getPrivateImageUrl(selected.verification.id_card_back, 'id_cards')"
                                            alt="CCCD Sau" />
                                ...
                                <!-- Ảnh selfie thực tế -->
                                <div class="doc-card"
                                    v-if="getPrivateImageUrl(selected?.verification?.face_auth_image, 'faces')">
                                    <span class="doc-lbl">Ảnh selfie thực tế</span>
                                    <div :class="['doc-img-wrap', !isKycUnlocked ? 'kyc-blurred' : '']"
                                        @contextmenu.prevent @dragstart.prevent
                                        @click="isKycUnlocked && openImage(getPrivateImageUrl(selected.verification.face_auth_image, 'faces'))">
                                        <img :src="getPrivateImageUrl(selected.verification.face_auth_image, 'faces')"
                                            alt="Ảnh selfie" />
                            ...
                            <div v-else class="empty-doc-msg">
                                ...
                                <span>Tài khoản chủ trọ đã được xác minh thành công. (Dữ liệu thử nghiệm không đính kèm
                                    tệp
                                    ảnh).</span>
                            </div>
                        </div>
                        ...
                        <div class="doc-section"
                            v-if="normalizeImages(selected?.boarding_house?.contract_images).length">
                            <h4 class="doc-title">
                                <i class="bi bi-file-earmark-text-fill text-blue-600"></i> Giấy Tờ Pháp Lý / Hợp Đồng Cơ
                                Sở
                            </h4>
                            <div class="doc-grid">
                                <div v-for="(path, idx) in normalizeImages(selected.boarding_house.contract_images)"
                                    :key="'c-' + idx" class="doc-card">
                        ...
                        <div class="doc-section" v-if="normalizeImages(selected?.boarding_house?.room_images).length">
                            ...
                            <div class="doc-grid">
                                <div v-for="(path, idx) in normalizeImages(selected.boarding_house.room_images)"
                                    :key="'r-' + idx" class="doc-card">
                        ...
                        <!-- BẢN ĐỒ & TỌA ĐỘ GPS -->
                        <div class="doc-section"
                            v-if="Number(selected?.boarding_house?.latitude) && Number(selected?.boarding_house?.longitude)">
                            ...
                            <div class="gps-box">
                                <div class="gps-info">
                                    <div>
                                        <p class="gps-dec">Thập phân: <span>{{
                                            Number(selected.boarding_house.latitude).toFixed(7) }}, {{
                                                    Number(selected.boarding_house.longitude).toFixed(7) }}</span></p>
                                        <p class="gps-dms">DMS: {{ toDMS(selected.boarding_house.latitude, true) }} , {{
                                            toDMS(selected.boarding_house.longitude, false) }}</p>
                                    </div>
                                    <button type="button"
                                        @click="copyGpsCoordinates(selected.boarding_house.latitude, selected.boarding_house.longitude)"
                                        class="btn-copy-gps">
                                        <i class="bi"
                                            :class="copiedGps ? 'bi-check-lg text-emerald-600' : 'bi-clipboard'"></i>
                                        {{ copiedGps ? 'Đã sao chép' : 'Sao chép' }}
                                    </button>
                                </div>
                                <div class="map-frame">
                                    <iframe width="100%" height="220" style="border:0" loading="lazy"
                                        :src="'https://maps.google.com/maps?q=' + selected.boarding_house.latitude + ',' + selected.boarding_house.longitude + '&z=18&output=embed'"></iframe>
                                </div>
                            </div>
                        </div>
                        <div class="doc-section"
                            v-else-if="selected?.boarding_house?.address_detail || selected?.boarding_house?.district">
                            <h4 class="doc-title">
                            ...
                            <div class="map-frame">
                                <iframe width="100%" height="200" style="border:0" loading="lazy"
                                    :src="'https://maps.google.com/maps?q=' + encodeURIComponent([selected.boarding_house.address_detail, selected.boarding_house.district, 'Ninh Bình'].filter(Boolean).join(', ')) + '&hl=vi&z=15&output=embed'"></iframe>
                            </div>
                        </div>
                    </div>
            ...
            <!-- Modal Zoom Ảnh -->
            <div v-if="showImageModal" class="modal-overlay" style="z-index:1100; background:rgba(0,0,0,0.85);"
                @click.self="showImageModal = false" @contextmenu.prevent @dragstart.prevent>
                <div style="position:relative; max-width:90vw; max-height:90vh;" @contextmenu.prevent
                    @dragstart.prevent>
                    <button @click="showImageModal = false"
                        style="position:absolute; top:-40px; right:0; color:#fff; font-size:28px; background:none; border:none; cursor:pointer; z-index:30;">
                        &times;
                    </button>
                    <div style="position:relative; display:inline-block; overflow:hidden; border-radius:8px;"
                        @contextmenu.prevent @dragstart.prevent>
                        <img :src="currentImageUrl"
                            style="max-width:90vw; max-height:85vh; object-fit:contain; border-radius:8px; pointer-events:none; user-select:none;"
                            @contextmenu.prevent @dragstart.prevent />
                        ...
                        <div style="position:absolute; inset:0; z-index:25; background:transparent;"
...
.doc-img-wrap:hover .img-overlay {
    opacity: 1;
}
