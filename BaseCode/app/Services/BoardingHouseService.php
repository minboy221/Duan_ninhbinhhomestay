...
class BoardingHouseService
{
    ...
    public function createBoardingHouse(array $data, $roomImages, $contractImages, $userId, $userName)
    {
        ...
        if ($contractImages) {
            foreach ($contractImages as $file) {
                $path = $file->store('boarding_houses/contracts', $disk);
        ...
        if ($roomImages) {
            foreach ($roomImages as $file) {
                $path = $file->store('boarding_houses/rooms', $disk);
                $roomImagesPath[] = $path;
            }
        }
    ...
    public function updateBoardingHouse(int $id, array $data, $roomImages, $contractImages, int $userId)
    {
        ...
        if ($contractImages) {
            ...
            foreach ($contractImages as $file) {
                $contractImagesPath[] = $file->store('boarding_houses/contracts', $disk);
            }
            $data['contract_images'] = $contractImagesPath;
        }
        ...
        if ($roomImages) {
            $roomImagesPath = [];
            foreach ($roomImages as $file) {
                $roomImagesPath[] = $file->store('boarding_houses/rooms', $disk);
            }
            $data['room_images'] = $roomImagesPath;
        }
